                ...
                <HeadingBox title="Guides" size="text-[16px]" class="h-[106px] w-full">
                    <div class="flex gap-[7px] -mt-2 -ml-2.5 w-full overflow-x-auto">
                        {#each guides as guide, i}
                            <a href="/guides/{guide.slug}" class="border-2 rounded-[13px] border-[#8A8A8A] flex items-center w-[179px] h-[76px] p-3 gap-[11px] flex-shrink-0 {i > 0 ? 'hidden sm:flex' : ''} {i > 1 ? 'hidden md:flex' : ''} {i > 1 ? 'hidden lg:flex' : ''} lg:flex">
                                <p>{guide.title}</p>
                            </a>
                        {/each}
